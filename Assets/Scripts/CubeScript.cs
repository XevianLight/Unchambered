...
public class CubeScript : MonoBehaviour
{
    ...
    void Update()
    {
		lossyScale = transform.lossyScale * inverseScaleFactor;
        if (lt != null)
        {
	    ...
	    if (!held && snappingEnabled && placeInArea && areaObject != null){
	    	...
	    	transform.localRotation = Quaternion.Lerp(transform.localRotation, Quaternion.Euler(Mathf.Round(transform.localEulerAngles.x / snapAngle) * snapAngle, Mathf.Round(transform.localEulerAngles.y / snapAngle) * snapAngle, Mathf.Round(transform.localEulerAngles.z / snapAngle) * snapAngle), Time.deltaTime * snapSpeed);
	    	transform.localScale = Vector3.Lerp(transform.localScale, new Vector3((Mathf.Round(transform.parent.transform.localScale.x*100)/100)/transform.parent.transform.localScale.x, (Mathf.Round(transform.parent.transform.localScale.y*100)/100)/transform.parent.transform.localScale.y, (Mathf.Round(transform.parent.transform.localScale.z*100)/100)/transform.parent.transform.localScale.z) * targetScale.x, Time.deltaTime * snapSpeed);
	    	...
	    	transform.localScale = Vector3.Lerp(transform.localScale, initialScale, Time.deltaTime * snapSpeed);
	    }
	    
	    if (areaObject) {
	    	if (childColliders){ //If the object has a child which contains its compound collider
